...
val filteredData=business.filter(line=>line(1).contains("TX")).map(line=>(line(0).toString,line(1).toString))

val review=sc.textFile("/yelpdatafall/review/review.csv").map(line=>line.split("\\^")) 

val count=review.map(line=>(line(2),1)).reduceByKey((a,b)=>a+b)
